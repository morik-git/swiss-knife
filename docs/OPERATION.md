...
- 成果物（結合済みCSV）と運用手順をGitHubに保存し、再現可能にする。
...
> 注：`/work` と `/data` は自動で存在するわけではありません。`docker run` の `-v <WindowsPath>:/work` / `-v <WindowsPath>:/data` を付けて起動した「そのコンテナの中」でのみマウントポイントとして存在します。
...
- ローカルPCの Python は Microsoft Store stub などで実体が無く、venv運用は行わない
...
- コンテナのプロンプトが `root@...` のときは Linux側。コンテナ内で `docker` は通常使えない
...
2. コンテナ内で `pip install -e /work`
3. 2行ヘッダー維持の `awk` で結合（出力は Downloads に生成）

---

## 1) コンテナ起動（swiss-knife と Downloads を同時に使う）
